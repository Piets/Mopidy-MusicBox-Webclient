...
function processGetPlaylists(resultArr) {
    ...
    for (var i = 0; i < resultArr.length; i++) {
        ...
        if(starredRegex.test(resultArr[i].uri)) {
    	    starred = '<li><a href="#" onclick="return showTracklist(this.id);" id="' + resultArr[i].uri + '""><h1>&#9733; Spotify Starred Tracks</h1></a></li>';
	} else {
        child = '<li><a href="#" onclick="return showTracklist(this.id);" id="' + resultArr[i].uri + '"><h1><i class="' + iconClass + '"></i> ' + resultArr[i].name + '</h1></a></li>';
        tmp += child;
    }
    };
